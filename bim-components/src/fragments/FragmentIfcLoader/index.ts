  ...
  implements Disposable, UI
{
  ...
  // For saving loaded models
  private loadedGroups: FragmentsGroup[] = [];
  ...
  // Getter for the loaded models
  getLoadedGroups() {
    return this.loadedGroups;
  }

  // Reset all saved models
  resetLoadedGroups() {
    this.loadedGroups = [];
  }
  ...
  async load(data: Uint8Array, name: string) {
    ...
    // Add the loaded model into the model groups
    this.loadedGroups.push(model);
